window.onload = () => {
    ...
    loginButton.addEventListener('click', loginClient);
};
const loginClient = () => {
    // event.preventDefault();
    let loginUserForm = document.getElementById('client-login-form');
    console.log('get form\'s elements');
    const elems = loginUserForm.elements;
    ...
    ajaxRequest.onreadystatechange = () => {
        if (ajaxRequest.readyState == 4 && ajaxRequest.status == 200) {
            ...
            if ( res.result === 'success'){
                ...
                window.location.replace('/clientHome.html');
            }
        }
    };
    ajaxRequest.open('POST', 'http://127.0.0.1:5000/chores/api/v1.0/auth/user/login', true);
